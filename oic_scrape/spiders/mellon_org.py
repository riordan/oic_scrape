    ...
    def parse_grant_details(self, response):
        ...
        if grant_start_date:
            try:
                ...
                grant_end_date = grant_start_date + relativedelta(
        ...
        a = AwardItem(
            ...
            recipient_org_location=details["location"],
            # pi_name=None,  # No PI is listed in their database.
            grant_year=int(grant_start_date.strftime("%Y"))
            if grant_start_date
            else None,
            ...
            award_amount=float(details["amount"]),
            award_currency="USD",  # Adjust if currency information is available
            award_amount_usd=float(details["amount"]),
            source="mellon.org",
            source_url=f"https://https://www.mellon.org/grant-details/{details['id']}",
            ...
            raw_source_data=str(details),  # Store the raw source data for debugging
